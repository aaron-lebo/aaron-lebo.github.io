...
However, why are so many people using Javascript on the serverside right now, anyway? Because they hear that Node and Javascript are fast. Compared to most other dynamic languages, this is objectively true. This stems from one of the great advantages Javascript has over pretty mcuh every other language. Millions of people are depending on it through their browsers, and numerous companies are tyring to reach those same people. They all benefit from it being performant. So you've got Microsoft, Mozilla, Google, Facebook, and several other companies pouring thousands of hours and millions of dollars into making it as fast as possible. Nothing else is getting that kind of support. This isn't relevant to just performance, it is relevant to new features, it is relevant to lbraries, tooling, etc. This advnatage can not be overstated, and it is not going anywhere because browsers are so reliant upon the language. We have COBOL code running on mainframes right now. We'll have Javascript running on websites 50 years from now, even if we have long since moved on to something else.
...
* an expressive and relatively decent sytax which is both easy to adapt to and won't drive anyone away immediately
